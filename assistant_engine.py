...
TOOLS = [
    ...
    },
    {
        ...
        "description": "Run the daily briefing: check the time, count unread emails, check light status, set lights to a time-appropriate mood, and deliver a personalized summary. Use when the user says good morning, briefing, catch me up, what did I miss, daily update, or similar.",
        "params": [],
    },
    {
        "name": "briefing.goodnight",
        "description": "Run the goodnight wind-down: dim lights to a sleepy warm glow and give a calming end-of-day message. Use when the user says good night, goodnight, wind down, bedtime, or time for bed.",
        "params": [],
    },
]
...
- "good morning" / "briefing" / "catch me up" / "what did I miss" / "daily update" → briefing.morning
- "good night" / "goodnight" / "wind down" / "bedtime" / "time for bed" → briefing.goodnight
...
"""
    ...
    def handle_message(self, user_text: str) -> str:
        ...
        if action == "briefing.morning":
            ...
            mood = get_time_of_day_light_mood()
            light_note = ""
            govee_style = self._pick_govee_style(mood)
            if govee_style:
                try:
                    set_lights_style(state="on", **govee_style)
                    light_note = "Lights have been set to a time-appropriate mood. "
                    self.log("Briefing: Govee lights set.")
                except LightsClientError as e:
                    self.log(f"Briefing Govee set failed: {e}")
            try:
                nanoleaf.turn_on()
                nl_style = self._pick_nanoleaf_style(mood)
                if nl_style:
                    ...
                    if nl_style.get("brightness") is not None:
                        nanoleaf.set_brightness(nl_style["brightness"])
                    self.log("Briefing: Nanoleaf set.")
            except Exception as e:
                self.log(f"Briefing Nanoleaf set failed: {e}")
            return self._call_model(
                user_text,
                None,
                extra_note=(
                    f"System note: The user asked for a daily briefing. {light_note}"
                    f"Here is the gathered data—use it to deliver a warm, personalized briefing "
                    f"in Galadrial's voice. Be concise (3–6 sentences). Mention the time, "
                    f"email highlights, and any relevant context for the time of day.\n\n"
                    f"{briefing_text}\n\n"
                ),
            )
        ...
        if action == "briefing.goodnight":
            ...
            if govee_style:
                try:
                    set_lights_style(state="on", **govee_style)
                    self.log("Goodnight: Govee dimmed.")
                except LightsClientError as e:
                    self.log(f"Goodnight Govee set failed: {e}")
            try:
                nanoleaf.turn_on()
                nl_style = self._pick_nanoleaf_style(mood)
                if nl_style:
